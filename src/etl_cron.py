...
response_url_base = requests.request(
    "GET", url_base, headers=headers, data=payload)
...
RAW_DIR_PATH = os.getenv("RAW_DIR_PATH", os.path.join(
    os.getcwd(), 'DataFrame', '01_RAW'))


def RAW_DIR(x): return os.path.join(
    RAW_DIR_PATH, re.sub('[^a-zA-Z0-9 \n\.]', '_', x + '.csv'))


TRUSTED_DIR_PATH = os.getenv("TRUSTED_DIR_PATH", os.path.join(
    os.getcwd(), 'DataFrame', '02_TRUSTED'))
...
data = json.loads(response_url_base.text.encode('utf8').decode(
    'utf8')) if response_url_base.status_code == 200 else print('Escrever Log')
...
url_final = "https://brasil.io/api/dataset/covid19/caso_full/data/?page=" + \
    str(page_final) + ""
...
response_url_final = requests.request(
    "GET", url_final, headers=headers, data=payload)
...
RAW_DIR_FILE = RAW_DIR(os.path.join(
    re.sub('[^a-zA-Z0-9 \n\.]', '_', url_base)))

pd.DataFrame.from_dict(data['results'])    .to_csv(RAW_DIR_FILE, index=False) if not re.sub(
    ...
    while link_num_begin < link_num_end:
        for link_num in range(link_num_begin, link_num_end):
            ...
            if re.sub('[^a-zA-Z0-9 \n\.]', '_', url_raw + str(link_num)) + '.csv' in os.listdir(RAW_DIR_PATH):
                ...
                response_url_link_num = requests.request(
                    "GET", url_raw + str(link_num), headers=headers, data=payload)
                data_link_num = json.loads(response_url_link_num.text.encode('utf8').decode(
                    'utf8')) if response_url_link_num.status_code == 200 else print('Escrever Log')

                print(url_raw + str(link_num))

                pd.DataFrame.from_dict(data_link_num['results'])                    .to_csv(RAW_DIR(url_raw + str(link_num)), index=False) if not re.sub(
    ...
    while data_link != None:
        for link_num in range(page_final - 1, page_final + 1):
            ...
            if re.sub('[^a-zA-Z0-9 \n\.]', '_', url_raw + str(link_num)) + '.csv' in os.listdir(RAW_DIR_PATH):
                ...
                pd.DataFrame.from_dict(data_link_num['results'])                    .to_csv(RAW_DIR(url_raw + str(link_num)) + '.csv', index=False) if not re.sub(
                    '[^a-zA-Z0-9 \n\.]', '_', url_raw + str(link_num)) + '.csv' in os.listdir(RAW_DIR_PATH) else print('DataFrame já carregado', end='\n\n')
...
# selecionando apenas arquivos
files = filter(os.path.isfile, os.listdir(RAW_DIR_PATH))
# adiciona o diretório completo em uma lista
files = [os.path.join(RAW_DIR_PATH, f) for f in files]
# ordernacao por ondem de data/hora
...
def norm(y): return y.replace('.csv', '').replace('.', '_')
...
for x in range(files.index(files[0]), (files.index(files[-1]) + 1)):
    ...
    tabelas['' + files[x].replace('.csv', '') + ''] = pd.read_csv(
        RAW_DIR_PATH + os.path.sep + files[x], delimiter=',')
...
df = pd.DataFrame(None, columns=list(
    eval(list(tabelas.keys())[0]).head()[0:0]))
...
for s in set(df.state):
    df[df.state == s].to_csv(os.path.join(
        TRUSTED_DIR_PATH, str(s.lower()) + '.csv'), index=False)
